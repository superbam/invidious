...
//
// These pages can render hundreds of cards in one response (a subscriptions
// feed easily hits 600), so fetches are gated by IntersectionObserver and
// only issued as cards approach the viewport — firing one request per card
// up front would queue behind the browser's per-origin connection limit and
// never finish for most of the page.
...
(function () {
        ...
        if (prefs.dearrow_thumbnails) {
            ...
            if (best_thumb) {
                ...
                document.querySelectorAll('img.thumbnail[data-video-id="' + id + '"]').forEach(function (img) {
                    // Preload so a not-yet-generated thumbnail (which fails)
                    // never clobbers the working default.
    ...
    var observer = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
            ...
            var id = entry.target.getAttribute('data-video-id');
            if (id) fetch_branding(id);
        });
